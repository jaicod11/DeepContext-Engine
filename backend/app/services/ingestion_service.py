...
class IngestionService:
    ...
    async def _load_pptx_paged(self, path: Path) -> list[tuple[str, dict]]:
        ...
        Key fix: join text RUNS with a space before joining paragraphs —
        prevents garbled output like "operations:ation" when adjacent runs
        inside a paragraph have no trailing/leading whitespace.
        """
        import asyncio
        ...
        def _read() -> list[tuple[int, str]]:
            ...
            for slide_num, slide in enumerate(prs.slides, start=1):
                # Use slide title if present as the first line
                title_text = ""
                if slide.shapes.title and slide.shapes.title.text.strip():
                    title_text = slide.shapes.title.text.strip()

                parts: list[str] = [f"Slide {slide_num}" + (f": {title_text}" if title_text else "")]

                for shape in slide.shapes:
                    if not shape.has_text_frame:
                        continue
                    for para in shape.text_frame.paragraphs:
                        # Join individual runs with a space — this prevents
                        # "word1word2" when runs have no whitespace padding.
                        run_texts = [run.text for run in para.runs if run.text.strip()]
                        para_text = " ".join(run_texts).strip()

                        # Fallback to para.text if runs are empty
                        # (some exporters put text directly on the paragraph)
                        if not para_text:
                ...
                # Include speaker notes — often contain full explanations
